...
interface SeoProductsProps {
  artworksConnection: SeoProductsForArtworks_artworks
}

export class SeoProducts extends React.Component<SeoProductsProps> {
  render() {
    const { artworksConnection } = this.props
    ...
    const artworksForSeoProduct =
      ...
      artworksConnection.edges.filter(edge => {
        return get(edge, e => {
          console.log("filtering edge", e)
          return e!.node!.is_acquireable && e!.node!.partner!.profile!.icon!.url
        })
      })

    if (!artworksForSeoProduct) {
      console.log("is null")
      return null
    } else {
                      ...
                      offers: {
                        ...
                        price: !is_price_range
                          ...
                          : {
                              minPrice:
                                display &&
                                formatCurrency(display.split("-")[0]),
                              maxPrice:
                                display &&
                                formatCurrency(display.split("-")[1]),
                            },
